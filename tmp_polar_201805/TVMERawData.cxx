...
void TVMERawData::MakeTree(const char *fname)
{
  ...
  fTree2 = new TTree("pp2", gVME->GetName());
  fTree2->SetAutoSave(10000000); // autosave (tree header) when 10M entries written
  fTree2->Branch("ch56", &ch56, "ch56/I");
  fTree2->Branch("ch57", &ch57, "ch57/I");
  fTree2->Branch("ch58", &ch58, "ch58/I");
  fTree2->Branch("ch59", &ch59, "ch59/I");
  fTree2->Branch("MSC", &fMSCCnt);
}
//______________________________________________________________________________
...
void TVMERawData::DecodeDataFile(FILE *file)
{
  ...
  //  don't 2x write
  //  if (fTree2) fTree2->GetCurrentFile()->Write();
...
void TVMERawData::DecodeSTRL()
{
  ...
  if (end) {
    if (fMSCChan != 16) {
      Printf("WRONGGGGGGGGGGGG SPILLLLLLLLLL %d", fMSCChan);
      fMSCCnt.Reset(); // all to zero
    }
    if (fTree2) fTree2->Fill();

    Printf("spills = %d, events = %d", fNSpills, fNEvents);
    //    Printf("ch56 = %d", ch56);
    //    Printf("ch57 = %d", ch57);
    //    Printf("ch58 = %d", ch58);
    //    Printf("ch59 = %d", ch59);
    Int_t delta = ch56+ch57+ch59-ch58; // ch58 is trigger
    if (delta != 0) Printf("delta ch58 = %d   !!!!!!!!!!!!!!!", delta);

    // reset
    fMSCChan = 0;
    ch56 = ch57 = ch58 = ch59 = 0;
  }
...
void TVMERawData::DecodeTLD()
{
  ...
  Int_t ch_tmp = fModule->GetFirstChannel() + fModule->MapChannel(id, ch);
  //  Printf("ch_tmp = %d", ch_tmp);
  if (ch_tmp == 56) ch56++;
  if (ch_tmp == 57) ch57++;
  if (ch_tmp == 58) ch58++;
  if (ch_tmp == 59) ch59++;
...
void TVMERawData::CheckIntegrity(ETypeStatus type, Bool_t status, const char *where)
{
  ...
  if (TestBit(type) == status) {
    Warning("CheckIntegrity", "%s after previous %s", where, where);
